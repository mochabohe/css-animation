...
### Available skills
- add-animation: 在 css-animation 项目中新增一个完整的 CSS 动画效果。当用户说「新增动画」、「添加动画」、「加一个动画」时使用。会同步修改 index.html、animations.css、data.js、snippets.js，并运行参数替换防回归检查。 (file: C:/Users/25198/Desktop/DC-Project/css-animation/.agents/skills/add-animation/SKILL.md)
- check: 在 css-animation 项目中运行完整质量检查（参数替换守卫 + ESLint + Vitest）。当用户说「跑一下检查」、「lint 一下」、「跑测试」、「检查一下代码」时使用。有报错会自动修复直到全部通过。 (file: C:/Users/25198/Desktop/DC-Project/css-animation/.agents/skills/check/SKILL.md)
