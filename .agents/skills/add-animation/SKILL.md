---
name: add-animation
description: "在 css-animation 项目中新增一个完整的 CSS 动画效果。当用户说「新增动画」、「添加动画」、「加一个动画」时使用。会同步修改 index.html、animations.css、data.js、snippets.js，并运行参数替换防回归检查。"
---

# 新增 CSS 动画

根据用户描述，在项目中新增一个完整的 CSS 动画效果。需要同步修改四个文件。
...
4. 读取 `css-animation/src/snippets.js`，找到同类动画的 snippet 写法（用于代码实验室）
...
## 第六步：修改 `css-animation/src/snippets.js`

在 `snippetsByTitle` 中添加同名条目，确保：
- key 与 `index.html` 的 `<h2>`、`data.js` 的动画名称完全一致
- snippet 中包含该动画对应的 `animation: keyframe duration timing ...`
- 若动画使用 `calc(var(--fx-duration) ...)`，保持与真实 CSS 一致，避免参数替换失效
...
cd css-animation && npm run -s test
cd css-animation && npm run -s lint
```

若失败，优先检查：
- `src/modal.js` 的 `DURATION_RE` 是否支持嵌套 `calc(...)`
- `animationParams.target` 与 snippet 中动画名是否一致
- 是否遗漏 `snippetsByTitle` 映射

## 第八步：完成后告知用户

列出修改了哪四个文件的哪些位置，并提醒运行：
