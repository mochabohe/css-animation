...
.hourglass {
  ...
  height: 60px;
  margin: 0 auto;
  animation: hgFlip var(--fx-duration, 2s) ease-in-out infinite;
}
.hourglass::after {
  content: "";
  position: absolute;
  ...
  animation: sandStream var(--fx-duration, 2s) linear infinite;
  pointer-events: none;
}
.hg-top {
  width: 36px;
...
.hg-top::after {
  ...
  border-radius: 0;
  animation: sandTop var(--fx-duration, 2s) linear infinite;
}
.hg-neck {
  width: 4px;
  height: 8px;
...
.hg-bot::after {
  ...
  border-radius: 0;
  animation: sandBottom var(--fx-duration, 2s) linear infinite;
}
@keyframes hgFlip {
...
@keyframes sandTop {
  ...
  90.01% { height: 75%; opacity: 0; }
  100% { height: 75%; opacity: 1; }
}
@keyframes sandBottom {
  0% { height: 0%; opacity: 1; }
  80% { height: 65%; opacity: 1; }
  90% { height: 65%; opacity: 0; }
  90.01% { height: 0%; opacity: 0; }
...
.planet-404 {
  ...
  justify-content: center;
  margin: 0 auto;
  animation: planetFloat var(--fx-duration, 3s) ease-in-out infinite;
}
.planet-orb {
  width: 52px;
  height: 52px;
  ...
  position: relative;
  background: radial-gradient(ellipse at 35% 35%, rgba(var(--accent-rgb), 0.5), rgba(var(--accent-rgb), 0.1) 70%);
}
.planet-orb::before {
  content: "";
  position: absolute;
  inset: -10px;
  background: repeating-linear-gradient(-25deg, transparent 0px, transparent 7px, rgba(var(--accent-rgb), 0.12) 7px, rgba(var(--accent-rgb), 0.12) 11px);
  animation: planetSpin calc(var(--fx-duration, 3s) * 2) linear infinite;
}
.planet-orb::after {
  content: "404";
  position: absolute;
...
.planet-star {
  ...
  animation: starTwinkle var(--fx-duration, 3s) ease-in-out infinite;
}
.planet-star--1 { top: 5px; right: 10px; }
.planet-star--2 { top: 20px; right: 3px; width: 2px; height: 2px; animation-delay: calc(var(--fx-duration, 3s) * -0.33); }
.planet-star--3 { bottom: 8px; right: 14px; width: 2px; height: 2px; animation-delay: calc(var(--fx-duration, 3s) * -0.66); }
...
@keyframes starTwinkle {
  ...
  迷失信号: `<!-- HTML -->
<div class="lost-signal">
  <span class="signal-arc signal-arc--1"></span>
  <span class="signal-arc signal-arc--2"></span>
  <span class="signal-arc signal-arc--3"></span>
...
.signal-arc {
  ...
  border-right: none;
  flex-shrink: 0;
  animation: signalFade var(--fx-duration, 2s) ease-in-out infinite;
}
.signal-arc--1 { width: 14px; height: 9px; animation-delay: 0s; }
.signal-arc--2 { width: 26px; height: 14px; animation-delay: calc(var(--fx-duration, 2s) * -0.2); }
.signal-arc--3 { width: 40px; height: 20px; animation-delay: calc(var(--fx-duration, 2s) * -0.4); }
...
@keyframes prismRingSweep { from { transform: rotate(0deg); } to { transform: rotate(-360deg); } }`,
  三角轮转点: `.triangle-cycle-loader {
  width: 76px;
  height: 76px;
  position: relative;
  margin: 0 auto;
}
.triangle-cycle-loader span {
  position: absolute;
  left: 50%;
  top: 50%;
  width: 14px;
  height: 14px;
  border-radius: 50%;
  animation: triangleCycleMotion calc(var(--motion-loading-duration) * 1.65 / var(--speed-multiplier, 1)) linear infinite;
}
.triangle-cycle-loader span:nth-child(1) {
  --p0x: -7.3px;
  --p0y: -6.6px;
  --p1x: 0px;
  --p1y: 22px;
...
.triangle-cycle-loader span:nth-child(2) {
  --p0x: 9.4px;
  --p0y: -3.1px;
  --p1x: -19px;
  --p1y: -11px;
...
.triangle-cycle-loader span:nth-child(3) {
  --p0x: -2.1px;
  --p0y: 9.7px;
  --p1x: 19px;
  --p1y: -11px;
...
@keyframes triangleCycleMotion {
  0%,
  1.333% {
    transform: translate(-50%, -50%) translate(var(--p0x), var(--p0y));
  }
  16.667% {
    transform: translate(-50%, -50%) translate(var(--p1x), var(--p1y));
  }
  18.667% {
    transform: translate(-50%, -50%) translate(var(--p1x), var(--p1y));
  }
  32%,
  33.333% {
    transform: translate(-50%, -50%) translate(var(--p2x), var(--p2y));
  }
  34.667% {
    transform: translate(-50%, -50%) translate(var(--p2x), var(--p2y));
  }
  50% {
    transform: translate(-50%, -50%) translate(var(--p3x), var(--p3y));
  }
  52% {
    transform: translate(-50%, -50%) translate(var(--p3x), var(--p3y));
  }
  65.333%,
  66.667% {
    transform: translate(-50%, -50%) translate(var(--p4x), var(--p4y));
  }
  68% {
    transform: translate(-50%, -50%) translate(var(--p4x), var(--p4y));
  }
  83.333% {
    transform: translate(-50%, -50%) translate(var(--p5x), var(--p5y));
  }
  85.333% {
    transform: translate(-50%, -50%) translate(var(--p5x), var(--p5y));
  }
